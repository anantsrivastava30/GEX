...
- **Stock Leaders** — per-stock CAN SLIM scan across the holdings of every tracked market and sector ETF: quarterly and annual earnings growth with multi-quarter sales/EPS acceleration, ROE, 52-week-high breakouts scored by the quality of their base, relative strength, and institutional sponsorship with an accumulation trend built from the project's own dated observations. Gated by the market-direction state, credited to each stock's leading group, warmed nightly so every candidate carries all seven letters; fresh breakout buy flags land in the signal feed with O'Neil's pivot-relative entry range and 7-8% stop attached.
