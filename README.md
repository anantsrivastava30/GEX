...
- **Stock Leaders** - per-stock CAN SLIM scan across the configured top holdings of every tracked market and sector ETF: quarterly and annual earnings growth with multi-quarter sales/EPS acceleration, ROE, 52-week-high breakouts scored by base quality, relative strength, and lagged institutional sponsorship observations. The market-direction gate controls qualified setup labels; fresh breakout events land in the signal feed with a trigger-relative entry range and an illustrative 7-8% stop measured from the current snapshot.
