    ...
    <div className="space-y-5">
      ...
      <Panel title="Contract and Horizon">
        <form onSubmit={price} className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="text-xs text-muted">Steps: {steps}<input type="range" min="2" max="50" value={steps} onChange={(event) => setSteps(Number(event.target.value))} className="mt-2 w-full accent-accent" /><span className="flex justify-between font-mono text-[12px] md:text-[10px] text-faint"><span>2</span><span>50</span></span></label>
